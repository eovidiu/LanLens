...
final class UserPreferences {
    ...
    private enum Defaults {
        ...
        static let apiToken = ""
        static let autoScanEnabled = false
    ...
    // MARK: - Fingerprinting Settings

    var fingerbankEnabled: Bool {
        didSet { defaults.set(fingerbankEnabled, forKey: Keys.fingerbankEnabled) }
    }

    /// Fingerbank API key for enhanced device identification.
    ///
    /// - Warning: This token is stored in UserDefaults, which is NOT secure storage.
    ///   For production use with sensitive tokens, consider using Keychain Services.
    var fingerbankAPIKey: String {
